...
contract('Fee', function([_, feeCollector, user]) {
  ...
  before(async function() {
    ...
    const providerAddress = await tokenProviderQuick(this.token.address);
    ...
    const rule1TokenProviderAddress = await tokenProviderQuick(
    ...
    const provider2Address = await tokenProviderQuick(this.token2.address);
    ...
    const USDT_PROVIDER = await tokenProviderQuick(this.usdt.address);
